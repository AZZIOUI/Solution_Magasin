...
public class InvoiceService
{
    ...
    public async Task<string> GenerateInvoiceHtmlAsync(int venteId)
    {
        ...
        if (vente == null)
        {
            throw new ArgumentException("Vente non trouv�e", nameof(venteId));
        }
        ...
        html.AppendLine($"            <div><span class='info-label'>Num�ro de facture:</span> INV-{vente.IdVente:D6}</div>");
        ...
        html.AppendLine($"            <div><span class='info-label'>T�l�phone:</span> {client?.TelClient ?? "N/A"}</div>");
        html.AppendLine($"        </div>");
        html.AppendLine($"        <div class='info-row'>");
        html.AppendLine($"            <div><span class='info-label'>Adresse de livraison:</span> {vente.AdresseLiv ?? client?.AdresseClient ?? "N/A"}</div>");
        html.AppendLine($"            <div><span class='info-label'>M�thode de paiement:</span> {payment?.Methode ?? "N/A"}</div>");
        html.AppendLine($"        </div>");
        html.AppendLine("    </div>");
        ...
        html.AppendLine("            <tr>");
        html.AppendLine("                <th>R�f�rence</th>");
        html.AppendLine("                <th>Produit</th>");
        html.AppendLine("                <th>Quantit�</th>");
